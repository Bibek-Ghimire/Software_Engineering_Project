...
import { FileText, Trash2, Edit2, Download, PlusCircle, XCircle, Upload, BookOpen, User } from "lucide-react";
...
const AddResource = () => {
  ...
  const handleFileChange = (e) => {
    ...
    if (!allowedTypes.includes(file.type)) return toast.error("Only PDF, PNG, JPG, DOC allowed");
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.title || !formData.description || (!formData.file && !editingId))
  ...
  const handleEdit = (res) => {
    ...
    setFormData({ title: res.title || "", description: res.description || "", file: null });
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-600 dark:text-gray-300 text-lg">
            {user?.role === "teacher" ? "Manage and share educational resources" : "Explore available learning materials"}
          </p>
        </div>
          ...
          <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-xl mb-10 border border-blue-100 dark:border-blue-800">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 bg-gradient-to-r from-blue-500 to-blue-600 rounded-full flex items-center justify-center">
                {editingId ? <Edit2 className="w-5 h-5 text-white" /> : <PlusCircle className="w-5 h-5 text-white" />}
              </div>
              ...
              {editingId && !formData.file && (
                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-200 dark:border-blue-800">
                  <p className="text-sm text-blue-700 dark:text-blue-300 flex items-center gap-2">
                    ...
                    Current file: {resources.find(r => r._id === editingId)?.fileUrl.split("/").pop()}
                  </p>
                </div>
              )}
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-gray-600 dark:text-gray-300 text-lg">Loading resources...</p>
          </div>
        ) : resources.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h3 className="text-xl font-semibold text-gray-600 dark:text-gray-300 mb-2">No Resources Yet</h3>
            <p className="text-gray-500 dark:text-gray-400">
              {user?.role === "teacher" ? "Start by adding your first resource above!" : "No resources have been shared yet."}
            </p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="bg-gradient-to-r from-blue-500 to-blue-600 p-6 text-white">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <h3 className="text-xl font-bold mb-2 leading-tight">{res.title}</h3>
                      <div className="flex items-center gap-2 text-blue-100">
                        <User className="w-4 h-4" />
                        <span className="text-sm">{res.teacher?.name || "Unknown"}</span>
                      </div>
                    </div>
                    <div className="ml-4">
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {user?.role === "teacher" && res.teacher?._id === user?._id && (
                      <>
                        <button
