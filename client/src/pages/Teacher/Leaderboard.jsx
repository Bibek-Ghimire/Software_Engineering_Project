...
import { ChevronDown, ChevronUp, User, Trophy, Star, Users, TrendingUp, BookOpen, Clock, DollarSign, Award } from "lucide-react";
...
const Leaderboard = () => {
  ...
  const getRankIcon = (index) => {
    switch (index) {
      ...
      case 1: return <Award className="w-8 h-8 text-gray-400" />;
      case 2: return <Award className="w-8 h-8 text-orange-500" />;
      default: return <div className="w-8 h-8 rounded-full bg-gradient-to-r from-gray-300 to-gray-400 flex items-center justify-center text-white font-bold text-sm">{index + 1}</div>;
    }
  };
...
<div className="flex min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-slate-800 dark:to-gray-900 transition-all duration-700">
  ...
  <div className="ml-64 min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-gray-900 dark:via-slate-800 dark:to-indigo-900 w-full">
        ...
        <div className="relative z-10 pt-16 pb-12 px-6">
          ...
          >
            <div className="inline-flex items-center gap-3 mb-6 px-6 py-3 bg-white/80 backdrop-blur-sm rounded-full shadow-lg">
              ...
              <span className="text-sm font-medium text-gray-700">Teacher Excellence Rankings</span>
            </div>
            ...
            <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
              Celebrating our top-performing educators and their outstanding contributions to learning
            </p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 mb-8">
        ...
        >
          {[
            { icon: Users, label: "Active Teachers", value: teachers.length, color: "text-blue-600" },
            { icon: BookOpen, label: "Total Courses", value: teachers.reduce((acc, t) => acc + t.coursesCreated, 0), color: "text-green-600" },
            { icon: TrendingUp, label: "Avg Engagement", value: (teachers.reduce((acc, t) => acc + t.engagementScore, 0) / teachers.length || 0).toFixed(1), color: "text-purple-600" },
            { icon: Star, label: "Top Score", value: Math.max(...teachers.map(t => t.totalScore), 0), color: "text-yellow-600" }
          ].map((stat, index) => (
            <div key={index} className="bg-white/70 backdrop-blur-sm rounded-xl p-4 shadow-lg border border-white/20">
                  ...
                  <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                     ...
                     <div className="flex items-center gap-4 cursor-pointer"
                      ...
                      <div>
                        <div className="flex items-center gap-3 mb-2">
                          <h3 className="text-2xl lg:text-3xl font-bold text-white">{teacher.name}</h3>
                          <span className="text-2xl">{config.icon}</span>
                        </div>
                        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r ${config.badge} shadow-lg`}>
                          <span className="text-sm font-semibold text-gray-700">{teacher.batch} Tier</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                      <div className="text-center">
                        ...
                        <p className="text-3xl font-bold text-white">{teacher.coursesCreated}</p>
                        <p className="text-sm text-white/80 font-medium">Courses</p>
                      </div>
                      <div className="text-center">
                        <div className="w-12 h-12 mx-auto mb-2 rounded-lg bg-white/20 backdrop-blur-sm flex items-center justify-center">
                          <TrendingUp className="w-6 h-6 text-white" />
                        </div>
                        <p className="text-3xl font-bold text-white">{teacher.engagementScore}</p>
                        <p className="text-sm text-white/80 font-medium">Engagement</p>
                      </div>
                      <div className="text-center col-span-2 lg:col-span-1">
                        <div className="w-12 h-12 mx-auto mb-2 rounded-lg bg-white/20 backdrop-blur-sm flex items-center justify-center">
                          <Star className="w-6 h-6 text-white" />
                        </div>
                        <p className="text-3xl font-bold text-white">{teacher.totalScore}</p>
                  ...
                  {teacher.courses && teacher.courses.length > 0 && (
                    <div className="mt-8">
                      ...
                      >
                        ...
                        <span>
                          {expanded[teacher._id] ? 'Hide Courses' : `View ${teacher.courses.length} Course${teacher.courses.length !== 1 ? 's' : ''}`}
                        </span>
                      </motion.button>
                      ...
                      {expanded[teacher._id] && (
                        ...
                        >
                          {teacher.courses.map((course) => {
                            const levelConfig = courseLevelConfig[course.level] || courseLevelConfig.Beginner;
                            return (
                              <motion.div
                                key={course._id}
                              ...
                              >
                                ...
                                <div className="flex flex-wrap items-center gap-3">
                                  <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold ${levelConfig.bg} ${levelConfig.text} border ${levelConfig.border}`}>
                                    {course.level}
                                  </span>
